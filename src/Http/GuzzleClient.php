...
class GuzzleClient
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['base_uri'])) {
            $config['base_uri'] = Utils::uriFor($config['base_uri']);
        }
    ...
    public function requestAsync(string $method, $uri = '', array $options = []): PromiseInterface
    {
        ...
        $request = new GuzzleRequest($method, $uri, $headers, $body, $version);
    ...
    private function buildUri(UriInterface $uri, array $config): UriInterface
    {
        ...
        return empty($uri->getScheme()) && !empty($uri->getHost())
            ? $uri->withScheme('http')
            : $uri;
    }
    ...
    private function applyOptions(RequestInterface $request, array &$options): RequestInterface
    {
        ...
        if (isset($options['query'])) {
            ...
            if ($expand) {
                $expand = collect($expand)
                    ->map(fn($v, $k) => class_exists($v) ? $k : $v)
                    ->map(fn($v) => \Str::of($v)->replace('.', ']['))
                    ->map(fn($v) => "expand[$v][]")
                    ->join('&');

                $value = "$value&" . urlencode($expand);
            }
